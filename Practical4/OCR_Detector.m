...
county = 0;
for r = 1:5:size(I,1)      % slide vertically every 5 pixels
    predictedRow=[];
    county = county + 1;
    countx = 0;
    for c = 1:5:size(I,2)  % slide horizontally every 5 pixels
        ...
        %digitIm = preprocessDigit(digitIm);  %centering the digit increased the accuracy from .36 to .76
        %imshow(digitIm)

        %we reshape the digit into a vector
        digitVector = reshape(digitIm, 1, []);
        
        
        [prediction maxi] =  SVMTesting_v2(digitVector, modelSVM);
...
%Accuracy is not as good as before (.36) idk why cause I couldn't change
%the values before and didn't know how to compare to the MINST dataset.

figure;
imagesc(map);           % visualize confidence values as colors
axis image; colorbar;
title('Classifier confidence map');

mapNorm = (map - min(map(:))) / (max(map(:)) - min(map(:)));
bw = mapNorm > 0.4;

figure;
imagesc(bw);           % visualize confidence values as colors
axis image; colorbar;
title('Classifier map mask');